...
#         assert(((fts.abs().sum(dim=1, keepdim=True) != 0).float() - mask.float()).abs().sum().item() == 0)
        ...
        ## LG comment out the lines below for PU subtraction    
        #if self.use_counts:
...
class HGCalPileUpLCBased(nn.Module):
    ...
    def __init__(self,
                 ...
                 for_inference=False,
                 **kwargs):
        super(HGCalPileUpLCBased, self).__init__(**kwargs)
        self.lc_conv = FeatureConv(lc_features_dims, 32)
    ...
    def forward(self, lc_points, lc_features, lc_mask):
        points = torch.cat(lc_points, dim=1)
        features = torch.cat(self.lc_conv(lc_features) * lc_mask,  dim=1)
        mask = torch.cat(lc_mask, dim=1)
